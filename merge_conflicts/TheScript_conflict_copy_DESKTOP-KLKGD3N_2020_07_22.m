...
Option.winSize = 0.4;              % size of the window
...
if Option.directionality == "2 direction" % hpc-hpc and hpc-pfc
    ...
    for i = 1:numResult % "expected ONE OUTPUT from dot indexing"
        Patterns(1,i).directionality = "hpc-hpc";
        Patterns(1,i).X_source = X_source{i};
        Patterns(1,i).X_target = X_target{1,i};
        Patterns(1,i).name = patternNames(i);
        
        
        Patterns(2,i).directionality = "hpc-pfc";
        Patterns(2,i).X_source = X_source{i};
        Patterns(2,i).X_target = X_target{2,i};
        Patterns(2,i).name = patternNames(i);
    end
end
...
if doFactorAnalysis
    ...
    for i = progress(1:4, 'Title', 'FA over patterns')
...
optionFieldsNames = cell2mat((fieldnames(Option)'));
